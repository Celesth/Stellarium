...
(function () {
  ...
  GM_addStyle(`
    ...
    @media (max-width: 480px) {
      ...
      .hypr-notif.hypr-in  { opacity: 1; transform: translateY(0) scale(1); }
      .hypr-notif.hypr-out { opacity: 0; transform: translateY(24px) scale(0.97); transition: opacity 0.22s ease-in, transform 0.22s ease-in; }
    }

    .hypr-notif.hypr-in  { opacity: 1; transform: translateX(0) scale(1); }
    .hypr-notif.hypr-out { opacity: 0; transform: translateX(calc(100% + 24px)) scale(0.96); transition: opacity 0.22s ease-in, transform 0.22s ease-in; }
    ...
    .hypr-header { display: flex; align-items: center; gap: 7px; padding: 10px 12px 8px; }

    .hypr-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
    ...
    .hypr-appname { font-size: 9.5px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: var(--hypr-fg-muted); flex: 1; }

    .hypr-tag { font-size: 9px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; padding: 2px 7px; border-radius: 4px; }
    ...
    .hypr-close {
      background: none; border: none; color: var(--hypr-fg-dim); cursor: pointer;
      font-size: 15px; line-height: 1; padding: 6px 8px; margin: -6px -6px -6px 0;
      transition: color 0.15s; font-family: inherit; -webkit-tap-highlight-color: transparent;
    }
    .hypr-close:hover { color: var(--hypr-fg); }

    .hypr-divider { height: 1px; background: var(--hypr-separator); margin: 0 12px; }

    .hypr-body { padding: 10px 12px 4px; display: flex; gap: 10px; align-items: flex-start; }

    .hypr-thumb { width: 46px; height: 46px; border-radius: 6px; object-fit: cover; flex-shrink: 0; border: 1px solid var(--hypr-border); background: rgba(255,255,255,0.04); }
    .hypr-thumb-placeholder { width: 46px; height: 46px; border-radius: 6px; flex-shrink: 0; background: rgba(179,162,213,0.08); border: 1px solid var(--hypr-border); display: flex; align-items: center; justify-content: center; font-size: 18px; opacity: 0.5; }

    .hypr-text { flex: 1; min-width: 0; }
    .hypr-title { font-size: 12px; font-weight: 600; color: var(--hypr-fg); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.3; margin-bottom: 3px; }
    .hypr-message { font-size: 10.5px; color: var(--hypr-fg-muted); line-height: 1.5; }

    .hypr-meta { display: flex; flex-wrap: wrap; gap: 6px 0; padding: 8px 12px 10px; }
    .hypr-meta-item { display: flex; align-items: center; gap: 5px; width: 50%; min-width: 0; }
    .hypr-meta-item.full { width: 100%; }
    .hypr-meta-label { font-size: 9px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--hypr-fg-dim); white-space: nowrap; }
    .hypr-meta-value { font-size: 10px; color: var(--hypr-fg-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0; }
    ...
    #kmn-send-btn {
      ...
      box-shadow: 0 0 0 1px rgba(179,162,213,0.08), 0 8px 24px rgba(0,0,0,0.55);
    ...
    @media (max-width: 480px) {
      #kmn-send-btn { width: 56px; height: 56px; border-radius: 14px; bottom: max(20px, env(safe-area-inset-bottom, 20px)); right: 16px; }
      #kmn-send-btn svg { width: 24px; height: 24px; }
    }

    #kmn-send-btn:hover { border-color: rgba(179,162,213,0.55); box-shadow: 0 0 0 1px rgba(179,162,213,0.18), 0 0 18px rgba(179,162,213,0.15), 0 8px 24px rgba(0,0,0,0.55); transform: translateY(-1px); }
    #kmn-send-btn:active { transform: scale(0.92); border-color: rgba(179,162,213,0.6); }
    #kmn-send-btn svg { width: 20px; height: 20px; }
    #kmn-send-btn.sending { border-color: rgba(179,162,213,0.6); animation: kmn-pulse 1s ease-in-out infinite; pointer-events: none; }
  ...
  const Notify = (() => {
    ...
    function show({ message, type = "info", duration = CONFIG.notifDuration, title, thumbnail, meta = [] }) {
      ...
      notif.querySelector(".hypr-close").addEventListener("click", () => { clearTimeout(timer); dismiss(); });
      ...
      notif.addEventListener("touchmove", e => {
        ...
        if (Math.abs(dx) > dy && Math.abs(dx) > 8) {
          notif.style.transition = "transform 0.05s linear, opacity 0.05s linear";
          notif.style.transform  = `translateX(${dx}px)`;
          notif.style.opacity    = String(Math.max(0, 1 - Math.abs(dx) / 160));
        }
      }, { passive: true });
      notif.addEventListener("touchend", e => {
        ...
        if (Math.abs(dx) > 80) { clearTimeout(timer); dismiss(); }
          ...
          notif.style.transition = "transform 0.25s cubic-bezier(0.16,1,0.3,1), opacity 0.25s ease";
  ...
  function scrapePost() {
    ...
    post.service = parts[1] ?? "unknown";
    post.userId  = parts[3] ?? "unknown";
    post.postId  = parts[5] ?? "unknown";
    post.site    = location.hostname;
    ...
    post.published = document.querySelector(".post__published time")?.getAttribute("datetime") ||
      document.querySelector("time[datetime]")?.getAttribute("datetime") || null;
    post.publishedDisplay = post.published ? new Date(post.published).toLocaleString() : "Unknown";

    const contentEl = document.querySelector(".post__content");
    post.contentText = contentEl?.innerText?.trim().slice(0, 500) || "";

    post.contentLinks = contentEl
    ...
    const mediaExts = /\.(jpg|jpeg|png|gif|webp|avif|mp4|webm|mov)(\?|$)/i;

    post.files = [...document.querySelectorAll(".post__files .post__thumbnail")]
      .map(el => {
        const link = el.querySelector("a");
        const img  = el.querySelector("img");
        return { url: link?.href || img?.src || null, name: link?.download || link?.href?.split("/").pop() || "file", thumb: img?.src || null };
      })
      .filter(f => f.url);

    if (!post.files.length) {
      post.files = [...document.querySelectorAll('a[href*="/data/"], a[download]')]
        .map(a => ({ url: a.href, name: a.download || a.href.split("/").pop() || "file", thumb: null }));
    }
    ...
    post.mediaFiles = post.files.filter(f => mediaExts.test(f.url));
    post.otherFiles = post.files.filter(f => !mediaExts.test(f.url));
  ...
  function buildPayloads(post) {
    ...
    const over   = post.mediaFiles.length > CONFIG.attachThreshold;

    const descParts = [];
    if (post.contentText) descParts.push(post.contentText);
    if (post.contentLinks.length) {
      descParts.push(post.contentLinks.map(l => `[${l.label}](${l.url})`).join("\n"));
    }
    ...
    if (over) {
      const rawLinks = post.mediaFiles.map(f => f.url).join("\n").slice(0, 4096);
      embeds.push({
        color:       CONFIG.accentColor,
        description: rawLinks,
        footer:      { text: `${post.mediaFiles.length} attachments (raw links) · ${CONFIG.footerText}` },
      });
    } else {
      post.mediaFiles.forEach((file, i) => {
        embeds.push({
          color:  CONFIG.accentColor,
          image:  { url: file.url },
          footer: { text: `${i + 1} / ${post.mediaFiles.length} · ${file.name}` },
        });
      });
    }

    const allOther = [
      ...post.otherFiles.map(f => `[${f.name}](${f.url})`),
      ...post.attachments.map(a => `[${a.name}](${a.url})`),
    ];
    if (allOther.length) {
      embeds.push({
        color:  CONFIG.embedColor,
        fields: [{ name: "✦ Other Files", value: allOther.slice(0, 15).join("\n").slice(0, 1024) }],
        footer: { text: CONFIG.footerText },
      });
    }
  ...
  async function run() {
    ...
    Notify.show({
      ...
      meta: [
        ...
        { label: "mode",    value: over ? "raw links" : "embeds" },
      ],
    });
  ...
  function injectButton() {
    ...
    btn.addEventListener("touchend", e => { e.preventDefault(); run(); });
